...
Lcam = mtx.dot(np.hstack((cv.Rodrigues(rvecs[0])[0],tvecs[0])))
px=100
py=100
Z=0
# from https://stackoverflow.com/questions/34140150/reverse-of-opencv-projectpoints
def pxl_to_pose(px, py, Z=0):
    X = np.linalg.inv(np.hstack((Lcam[:,0:2] ,np.array([[-1*px],[-1*py],[-1]])))).dot((-Z*Lcam[:,2]-Lcam[:,3]))

side_length = 0.05 # side length of checkerboard in meters
xy = (X * side_length)[:2]


translation = []
angle = np.atan2(translation, height) 
# translations = 
